...
var requestToken = function(storeHostname) {
	return new Promise((resolve, reject) => {
		request({
			method: 'POST',
			url: arbiterURL + '/token',
			json: { target: storeHostname },
			headers: { 'X-Api-Key': arbiterToken },
			agent: httpsAgent
		}, (err, res, body) => {
			if (err || res.statusCode !== 200) {
...
var makeStoreRequest = function() {
	...
	return (options) => {
		var storeHostname = url.parse(options.url).hostname;

		return new Promise((resolve, reject) => {
			Promise.resolve().then(() => {
				if (!(storeHostname in tokenCache))
					return requestToken(storeHostname);
				return tokenCache[storeHostname];
			}).then((token) => {
				options.headers = options.headers || {};
				options.headers['X-Api-Key'] = tokenCache[storeHostname] = token;
...
exports.timeseries = {
	...
	since: function(href, dataSourceID, timestamp) {
		return makeStoreRequest({
			method: 'GET',
			json: true,
			url: href + '/' + dataSourceID + '/ts/since/' + timestamp,
			agent: httpsAgent
		});
	},
	...
	range: function(href, dataSourceID, startTimestamp, endTimestamp) {
		return makeStoreRequest({
			method: 'GET',
			json: true,
			url: href + '/' + dataSourceID + '/ts/range/' + startTimestamp + '/' + endTimestamp,
			agent: httpsAgent
		});
	},
...
exports.notifications = {
	...
	connect: function (href) {
		return new Promise((resolve, reject) => {
			...
			requestToken(storeURL.hostname).then((token) => {
